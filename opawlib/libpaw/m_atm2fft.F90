...
&                  pawtab,ph1d,qgrid,ucvol,vspl,optn2,&
&                  is2_in,comm_fft,me_g0,paral_kgb) ! optional arguments
...
!scalars
 integer,parameter :: &
 optatm=1,optdyfr=0,opteltfr=0,optgr=0,optn=1,optstr=0,optv=1,usepaw=1
...
!arrays
 ...
 integer  :: qprtrb(3)=0
 real(dp) :: gauss(2,ntypat*(optn2/3))
 real(dp),intent(in) :: gmet(3,3),gprimd(3,3)
 real(dp),intent(in) :: ph1d(2,3*(2*mgfft+1)*natom),qgrid(mqgrid)
 real(dp) :: rhog(2,nfft*optv*max(optgr,optstr,optdyfr,opteltfr))
 real(dp) :: vg(2,nfft*optn*max(optgr,optstr,optdyfr,opteltfr))
 real(dp) :: vg1(2,nfft*optn*opteltfr),vg1_core(2,nfft*optn*opteltfr)
 real(dp),intent(in) :: vspl(mqgrid,2,ntypat*optv)
 real(dp) :: vprtrb(2)=0d0
 real(dp),intent(out) :: atmrho(nfft*optn)
 real(dp),intent(inout) :: atmvloc(nfft*optv)
 real(dp) :: dyfrn(3,3,natom*optn*optdyfr),dyfrv(3,3,natom*optv*optdyfr)
 real(dp) :: eltfrn(6+3*natom,6)
 real(dp) :: grn(3,natom*optn*optgr)
 real(dp) :: grv(3,natom*optv*optgr),strn(6*optn*optstr)
 real(dp) :: strv(6*optv*optstr)
...
!Get the distrib associated with this fft_grid
 ...
 if (present(is2_in)) then
   if(is2_in<1.or.is2_in>6) then
     stop "is2_in must be between 1 and 6"
   else
     ndir = 1
   end if
   ndir = -1
...
!Zero out arrays to permit accumulation over atom types
 ...
 if (optv==1.and.optgr==1) then
   ALLOCATE(grv_indx(3,natom))
   grv_indx(:,:)=zero
 end if
 if (optn==1.and.optgr==1) then
   ALLOCATE(grn_indx(3,natom))
   grn_indx(:,:)=zero
 end if
 if (optv==1.and.optdyfr==1) then
   ALLOCATE(dyfrv_indx(3,3,natom))
   dyfrv_indx(:,:,:)=zero
 end if
 if (optn==1.and.optdyfr==1) then
   ALLOCATE(dyfrn_indx(3,3,natom))
   dyfrn_indx(:,:,:)=zero
 end if
...
!     tvalespl => psps%nctab(itypat)%tvalespl
     stop 'atm2fft not used with usepaw/=1'
   end if
