...
!&      (paw_ij(1)%has_dijhat==0.and.pawprtvol/=0)) then
     if (usexcnhat==0) then
       ...
       ALLOCATE(v_dijhat(cplex*nfft,nspden))
       v_dijhat_allocated=.true.
       v_dijhat = vtrial
     end if
!   end if
...
!   write(2200+iatom,*) 'pawij%has_dijhartree',paw_ij(iatom)%has_dijhartree
...
!   write(2400+iatom,*) 'Dij, all components'   
!   write(2500+iatom,*) 'Dij, cumulative'   
...
!     write(2400+iatom,*) 'dij0'   
!     write(2500+iatom,*) 'dij0'   
!     write(2400+iatom,*) dij0
...
!       write(2200+iatom,*) 'dijhartree_need',dijhartree_need
!       write(2200+iatom,*) 'dijhartree_available',dijhartree_available

     LIBPAW_ALLOCATE(dijhartree,(cplex_rf*lmn2_size))
!    ===== DijHartree already computed
     if (paw_ij(iatom)%has_dijhartree==2) then
!       write(2200+iatom,*) 'dijhartree already computed'
       dijhartree(:)=paw_ij(iatom)%dijhartree(:)
     else
!    ===== Need to compute DijHartree
!       write(2200+iatom,*) 'need to compute dijhartree'
       if (ipositron/=1) then
...
!       write(2400+iatom,*) 'dijhartree'
!       write(2500+iatom,*) 'dijhartree'
!       write(2400+iatom,*) dijhartree
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!    write(6001,*) 'vxc1',paw_an(iatom)%vxc1
!    write(6001,*) 'vxct1',paw_an(iatom)%vxct1

!    ===== Dijxc already computed
     if (paw_ij(iatom)%has_dijxc==2) then
       if (dij_need) paw_ij(iatom)%dij(:,:)=paw_ij(iatom)%dij(:,:)+paw_ij(iatom)%dijxc(:,:)
!       write(2400+iatom,*) 'Dijxc already computed'
     else
!       write(2400+iatom,*) 'Dijxc not already computed'
...
!       write(2400+iatom,*) 'dijhat'
!       write(2500+iatom,*) 'dijhat'
!       write(2400+iatom,*) dijhat
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!       
!!       write(2400+iatom,*) 'dijnd'
!!       write(2500+iatom,*) 'dijnd'
!!       write(2400+iatom,*) dijnd
!!       write(2500+iatom,*) paw_ij(iatom)%dij
!       
...
!       write(2400+iatom,*) 'dijso'
!       write(2500+iatom,*) 'dijso'
!       write(2400+iatom,*) dijso
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!       write(2400+iatom,*) 'dijpawu'
!       write(2500+iatom,*) 'dijpawu'
!       write(2400+iatom,*) dijpawu
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!       write(2400+iatom,*) 'dijexxc'
!       write(2500+iatom,*) 'dijexxc'
!       write(2400+iatom,*) dijexxc
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!       write(2400+iatom,*) 'dijexchat'
!       write(2500+iatom,*) 'dijexchat'
!       write(2400+iatom,*) paw_ij(iatom)%dijxc_hat
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
!       write(2400+iatom,*) 'dijexcval'
!       write(2500+iatom,*) 'dijexcval'
!       write(2400+iatom,*) dijxcval
!       write(2500+iatom,*) paw_ij(iatom)%dij
...
! *************************************************************************
! write(2201,*) 'calculating dijhartree'
!Useful data
...
!Initialization
 dijhartree=zero
! write(2201,*) 'eijkl',pawtab%eijkl
! write(2201,*) 'dltij',pawtab%dltij
! write(2201,*) 'nspdiag',nspdiag
! write(2201,*) 'pawrhoij%nrhoijsel',pawrhoij%nrhoijsel
! write(2201,*) 'pawrhoij%rhoijselect',pawrhoij%rhoijselect
! write(2201,*) 'pawrhoij%rhoijp',pawrhoij%rhoijp
 do ispden=1,nspdiag
   jrhoij=1
...
!     write(2201,*) 'ro',ro
!     write(2201,*) 'pawdltij',pawtab%dltij(klmn)
...
! write(6000,*) 'pawdijhartree',dijhartree
...
!Useful data
!write(870,*) 'calling pawdijhat'
!write(871,*) 'calling pawdijhat'
 lm_size=pawtab%lcut_size**2
! write(460,*) 'lm_size',lm_size
 lmn2_size=pawtab%lmn2_size
 nfgd=pawfgrtab%nfgd
 has_phase=.false.
...
!               write(870,*) 'ilslm,ic,gylm(ic,ilslm)',ilslm,ic,pawfgrtab%gylm(ic,ilslm)
!               write(870,*) 'prod(ilslm)',prod(ilslm)
             end do
           end do
         else
           ilslm1=1
...
!               write(871,*) 'klmn,ils,mm',klmn,ils,mm
!               write(871,*) 'dijhat_idij',dijhat_idij(klmn)
             end do
           end do
         end do
       else
...
!arrays
 ...
 real(dp),pointer :: dij2p(:),dij2p_(:)
  integer :: rank,merr
  call mpi_comm_rank(0,rank,merr)
  if(merr/=0) stop 'merr nonzero'
! *************************************************************************
