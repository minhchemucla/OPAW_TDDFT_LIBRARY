...
!  Allocations of "on-site" densities
   ...
   ALLOCATE(nhat1(cplex*mesh_size,lm_size,nspden*usenhat))
   rho1(:,:,:)=zero;trho1(:,:,:)=zero;nhat1(:,:,:)=zero
   if (ipositron/=0) then ! Additional allocation for the electron-positron case
     ALLOCATE(rho1_ep (cplex*mesh_size,lm_size,nspden))
     ALLOCATE(trho1_ep(cplex*mesh_size,lm_size,nspden))
     ALLOCATE(nhat1_ep(cplex*mesh_size,lm_size,nspden*usenhat))
   end if
...
!    These should already be allocated in paw_an_init!
     if (allocated(paw_an(iatom)%vxc1))  then
       DEALLOCATE(paw_an(iatom)%vxc1)
     end if
     if (pawxcdev==0)then
       ALLOCATE(paw_an(iatom)%vxc1(cplex*mesh_size,paw_an(iatom)%angl_size,nspden))
     else
       ALLOCATE(paw_an(iatom)%vxc1(cplex*mesh_size,lm_size,nspden))
     end if
...
!   write(12,*) 'usenhat',usenhat
!   write(12,*) 'rhoij',pawrhoij(iatom)%rhoijp

!   write(7,*) 'Calling pawdensities once'

!   write(9,*) 'rho1',size(rho1(:,1,1)),cplex
!   do iwww=1,lm_size 
!       write(9,*) rho1(:,iwww,:)
!   enddo
...
!     ALLOCATE(lmselect_ep(lm_size))
!     ALLOCATE(lmselect_cur_ep(lm_size))
...
!     DEALLOCATE(lmselect_ep)
!     DEALLOCATE(lmselect_cur_ep)
!   end if
...
!  Temporary storage
   ...
   if (pawxcdev/=0) then
     ALLOCATE(vxc_tmp(cplex*mesh_size,lm_size,nspden))
     if (need_kxc) then
       ALLOCATE(kxc_tmp(mesh_size,lm_size,nkxc1))
     end if
     if (need_k3xc) then
   ...
   if (pawxcdev==0) then
     ALLOCATE(vxc_tmp(cplex*mesh_size,pawang%angl_size,nspden))
     vxc_tmp(:,:,:)=zero
     if (need_kxc) then
       ALLOCATE(kxc_tmp(mesh_size,pawang%angl_size,nkxc1))
     end if
     if (need_k3xc) then
       ALLOCATE(k3xc_tmp(mesh_size,pawang%angl_size,nk3xc1))
     end if
   end if
   idum=0
   if (.not.allocated(kxc_tmp))  then
     ALLOCATE(kxc_tmp(0,0,0))
   end if
   if (.not.allocated(k3xc_tmp))  then
     ALLOCATE(k3xc_tmp(0,0,0))
   end if

!  ===== Vxc1 term =====
!   write(3,*) 'ipositron,pawxcdev,ipert',ipositron,pawxcdev,ipert
   if (ipositron/=1) then
     if (pawxcdev/=0) then
       if (ipert==0) then
...
!    ===== Re-compute a partial "on-site" density n1 (only l=lexexch contrib.)
     ALLOCATE(rho1xx(mesh_size,lm_size,nspden))
     ALLOCATE(lmselect_tmp(lm_size))
...
!     ALLOCATE(dij_ep(cplex*lmn2_size))
...
!   write(179,*) 'in den_pot',paw_ij(iatom)%has_dijhartree
   if (ipositron/=1) then
...
!   if (any(abs(nucdipmom(:,iatom))>tol8).and.ipert==0.and.ipositron/=1.and.option/=1) then
!     if(cplex/=1) then
!         write(*,*)'BUG in pawdenpot: pawrhoij must be complex for ND moments!'
!         stop
!     endif
...
!  Compute spin-orbit contribution to Dij
   if (pawspnorb>0.and.ipert==0.and.ipositron/=1.and.(option/=2.or.cplex_rhoij==2)) then
     if(cplex/=1) then
        write(*,*)'BUG in pawdenpot: Dij^SO not available for cplex=2!'
        stop
     endif
...
!  Compute contribution to on-site energy
   if (option/=1.and.pawspnorb>0.and.ipert==0.and.ipositron/=1.and.cplex_rhoij==2) then
     if(cplex/=1) then
         write(*,*) 'BUG in pawdenpot: Epaw^SO not available for cplex=2!'
         stop
     endif
     if(pawrhoij(iatom)%nspden/=4) then
         write(*,*)'BUG in pawdenpot: pawrhoij must have 4 components!'
         stop
     endif
     jrhoij=2 !Select imaginary part of rhoij
     do irhoij=1,pawrhoij(iatom)%nrhoijsel
...
!  ======================================================================
   ...
   if (pawtab(itypat)%useexexch>0.and.ipert==0.and.ipositron/=1) then
     ...
     if(paw_ij(iatom)%nspden==4) then
         write(*,*)'BUG in pawdenpot: Local ex-exch. not implemented for nspden=4!'
         stop
     endif
...
!  Computation of Fock contribution to Dij
   if (usefock==1) then
       ...
       if(cplex/=1) then
           write(*,*) 'BUG in pawdenpot: Dij^Fock not available for cplex=2!'
           stop
       endif

     if (ipositron/=1) then
!      Fock contribution to Dij
       ALLOCATE(dijfock_vv(cplex_dij*lmn2_size,ndij))
       ALLOCATE(dijfock_cv(cplex_dij*lmn2_size,ndij))
...
&                         +ro(2)*(half*dijfock_vv(kklmn+1,ispden)+dijfock_cv(kklmn+1,ispden))
       ...
       DEALLOCATE(dijfock_vv)
       DEALLOCATE(dijfock_cv)
...
!Various inits
 ...
 if (present(one_over_rad2)) then
   ...
   ALLOCATE(one_over_rad2_(mesh_size))
...
!               write(700,*) 'll,ilm',ll,ilm
!               write(700,*) 'ro_rg',ro_rg
!               write(700,*) 'phiphj',pawtab%phiphj(:,kln)*one_over_rad2_(:)
             end if
           end if
         end do  ! End loops over ll,lm
       end do
...
!  -- Print out moments of densities (if requested)
   if (abs(pawprtvol)>=2.and.opt_print==1.and.opt_dens<2) then
     ALLOCATE(aa(cplex*mesh_size))
     ALLOCATE(bb(cplex*mesh_size))
     if (opt_dens==0) then
