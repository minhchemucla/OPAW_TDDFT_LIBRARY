...
!!  It defines a structured datatype (libxc_functional_type) and associated methods
!!  to initialize/finalize it and get properties from it.
!!  * It uses by default a global variable (paw_xc_global, libxc_functional_type) which has
!!    to be initialized/finalized with the libxc_functionals_init and libxc_functionals_end methods.
...
!Public functions
 ...
 public :: libxc_functionals_get_hybridparams   ! Retrieve parameter(s) of a hybrid functional
 public :: libxc_functionals_set_hybridparams   ! Change parameter(s) of a hybrid functional
...
!Private global XC functional
 type(libxc_functional_type),target,save :: paw_xc_global(2)
...
!
 interface
   subroutine xc_get_singleprecision_constant(xc_cst_singleprecision) &
...
!
 interface
   subroutine xc_get_family_constants(xc_cst_unknown,xc_cst_lda,xc_cst_gga, &
...
!
 interface
   type(C_PTR) function xc_func_type_malloc() &
...
!
 interface
   subroutine xc_func_type_free(xc_func) &
...
!
 interface
   type(C_PTR) function xc_get_info_name(xc_func) &
...
!
 interface
   type(C_PTR) function xc_get_info_refs(xc_func,iref) &
...
!
 interface
   integer(C_INT) function xc_get_info_flags(xc_func) &
...
!
 interface
   integer(C_INT) function xc_get_info_kind(xc_func) &
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_constants_load
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_check
!! NAME
...
#if defined LIBPAW_ISO_C_BINDING
write(*,*) 'check flag 1'
 if (.not.libpaw_xc_constants_initialized) call libxc_functionals_constants_load()
write(*,*) 'check flag 2'
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_init
!! NAME
...
write(*,*) 'paw_xc_global%id' 
write(*,*) paw_xc_global(1)%id 
write(*,*) paw_xc_global(2)%id 
 write(*,*) 'init flag 2'
...
&      'Invalid IXC = ',ixc,char(10),&
&      'The LibXC functional family ',xc_func%family,&
&      'is currently unsupported by LibPAW',char(10),&
&      '(-1 means the family is unknown to the LibXC itself)',char(10),&
&      'Please consult the LibXC documentation',char(10)
     MSG_ERROR(msg)
   end if
! write(*,*) 'init flag 4'
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_end
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_fullname
!! NAME
...
! *************************************************************************
 ...
 if (xc_funcs(1)%id == 0) then
   if (xc_funcs(2)%id /= 0) then
     ...
     call char_c_to_f(strg_c,libxc_functionals_fullname)
   end if
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_family_from_id
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_getid
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_ixc
!! NAME
...
! *************************************************************************
 ...
 if (present(xc_functionals)) then
   ...
   libxc_functionals_ixc=paw_xc_global(1)%abi_ixc
 end if
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_isgga
!! NAME
...
! *************************************************************************
 ...
 if (present(xc_functionals)) then
   libxc_functionals_isgga=(any(xc_functionals%family==LIBPAW_XC_FAMILY_GGA) .or. &
&                      any(xc_functionals%family==LIBPAW_XC_FAMILY_HYB_GGA))
 else
   libxc_functionals_isgga=(any(paw_xc_global%family==LIBPAW_XC_FAMILY_GGA) .or. &
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_ismgga
!! NAME
...
! *************************************************************************
 ...
 if (present(xc_functionals)) then
   libxc_functionals_ismgga =(any(xc_functionals%family==LIBPAW_XC_FAMILY_MGGA))
 else
   libxc_functionals_ismgga =(any(paw_xc_global%family==LIBPAW_XC_FAMILY_MGGA))
 end if
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_is_hybrid
!! NAME
...
! *************************************************************************
 ...
 if (present(xc_functionals)) then
   libxc_functionals_is_hybrid=(any(xc_functionals%family==LIBPAW_XC_FAMILY_HYB_GGA) .or. &
&                          any(xc_functionals%family==LIBPAW_XC_FAMILY_HYB_MGGA))
 else
   libxc_functionals_is_hybrid=(any(paw_xc_global%family==LIBPAW_XC_FAMILY_HYB_GGA) .or. &
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_has_kxc
!! NAME
...
! *************************************************************************
 ...
 do ii=1,2
   if (present(xc_functionals)) then
     ...
     if (.not.paw_xc_global(ii)%has_fxc) libxc_functionals_has_kxc=.false.
   end if
 end do
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_nspin
!! NAME
...
! *************************************************************************
 ...
 if (present(xc_functionals)) then
   ...
   if (any(paw_xc_global%nspin==2)) libxc_functionals_nspin=2
 end if
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_getvxc
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_get_hybridparams
!! NAME
...
!!****f* libpaw_libxc/libxc_functionals_set_hybridparams
!! NAME
...
!!****f* m_libpaw_libxc_funcs/libxc_functionals_gga_from_hybrid
!! NAME
...
!  C is computed
     ...
     write(msg,'(2a,f9.6)' ) char(10),'In the mGGA functional TB09, c = ',cc
     call wrtout(std_out,msg,'COLL')
   end if
...
& libxc_functionals_check            => libxc_functionals_check, &
& libxc_functionals_init             => libxc_functionals_init, &
& libxc_functionals_end              => libxc_functionals_end, &
& libxc_functionals_fullname         => libxc_functionals_fullname, &
& libxc_functionals_getid            => libxc_functionals_getid, &
& libxc_functionals_family_from_id   => libxc_functionals_family_from_id, &
& libxc_functionals_ixc              => libxc_functionals_ixc, &
& libxc_functionals_getvxc           => libxc_functionals_getvxc, &
& libxc_functionals_isgga            => libxc_functionals_isgga, &
& libxc_functionals_ismgga           => libxc_functionals_ismgga, &
& libxc_functionals_is_hybrid        => libxc_functionals_is_hybrid, &
& libxc_functionals_has_kxc          => libxc_functionals_has_kxc, &
& libxc_functionals_nspin            => libxc_functionals_nspin, &
& libxc_functionals_get_hybridparams => libxc_functionals_get_hybridparams, &
& libxc_functionals_set_hybridparams => libxc_functionals_set_hybridparams, &
& libxc_functionals_gga_from_hybrid  => libxc_functionals_gga_from_hybrid
#endif
