...
!Mirror pseudopotential parameters to the output and log files
 ...
 if(rank==0) call wrtout(ab_out,msg,'COLL')
 if(rank==0) call wrtout(std_out,  msg,'COLL')
...
!If ncore mesh size is big, spline tncore into a smaller log. mesh
 ...
 write(*,*) 'reduced_ncore',reduced_ncor
...
!   do jlmn=1,size(vlocr)
!     write(78,*) vloc_mesh%rad(jlmn),vlocr(jlmn)
!   enddo
