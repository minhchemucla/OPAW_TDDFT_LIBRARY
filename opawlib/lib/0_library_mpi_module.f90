module mpi_lib_ours
  ...
  integer, parameter :: mpi_mx=1 !MPI_MAX ! DN
...
contains
  ...
  subroutine bcast_char264(a, iin)
    implicit none
    integer iin,ir
    character(len=*) :: a
    integer n
    ...
    call sync_mpi
    call mpi_bcast(a, n, mpi_cr, iin, mpi_comm_wd, ir);   call check(ir,0,' mpircv' )
    call sync_mpi
  end subroutine bcast_char264
  ...
  subroutine gatherv_c16(g, p, n, nlongmx, root)
    implicit none
    ...
    call sync_mpi
    call mpi_gatherv(g, n, mpi_double_cx, p, size_each, displs, mpi_double_cx, root, mpi_comm_wd, ir)
    call sync_mpi
    call check0(ir,' gatherv_ir ')

    deallocate(displs, size_each)
  end subroutine gatherv_c16

  subroutine scatterv_c16(p, g, nlongmx, n, root)
    implicit none
    integer n,st,nds, root, nlong, nlongmx, sz1(1), ir, i
    complex*16  g(n), p(*)
    integer, allocatable :: displs(:), size_each(:)
    ...
    call sync_mpi
    call mpi_scatterv(p, size_each, displs, mpi_double_cx, g, n, &
                                   mpi_double_cx, root, mpi_comm_wd, ir)
  ...
  subroutine alltoall_r8(p, g, n) ! transpose
    implicit none
    integer n, ir
    real*8 p(n), g(n)
    call mpi_alltoall(p,n,mpi_double_pn,  &   ! send buf,count,type
                      g,n,mpi_double_pn, &    ! recv buf,count,type
                      mpi_comm_wd, ir)   ! comm,flag
  ...
  subroutine mpi_max_r8(a,n)
    ...
    allocate(b(n),stat=ir);  if(ir/=0) stop ' bals '
    call sync_mpi
    call mpi_allreduce(a, b, n, mpi_double_pn, mpi_mx, mpi_comm_wd, ir);if(ir/=0) stop ' alsm '
    !write(6,*)' postchecking: rank=',rank; call flush(6);
    call sync_mpi
    a=b
    deallocate(b)

  end subroutine mpi_max_r8
