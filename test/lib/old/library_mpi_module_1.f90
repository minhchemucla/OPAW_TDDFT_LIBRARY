...
contains
  ...
  subroutine reduce_sum_r8(a,n,root)
    ...
    call mpi_reduce(a, b, n, mpi_double_precision, &
         mpi_sum, root, mpi_comm_world, ir);if(ir/=0) stop ' alsm '
    call sync_mpi
    if(rank==root) a=b
    deallocate(b)
  end subroutine reduce_sum_r8

  subroutine reduce_sum_c16(a,n,root)
    implicit none
    integer n,root,ir
    complex*16  a(n)
    ...
    allocate(b(n),stat=ir);  if(ir/=0) stop ' breds '
    call sync_mpi
    call mpi_reduce(a, b, n, mpi_double_complex, mpi_sum, root, mpi_comm_world, ir)
    if(ir/=0) stop ' alsm '
    call sync_mpi
    if(rank==root) a=b
    deallocate(b)
  end subroutine reduce_sum_c16
  ...
  subroutine allsum_scalar_c16(a)
    implicit none
    complex*16 a
    complex*16 a1(1)
    integer n
    ...
    n=1
    a1(1)=a
    call allsum_c16(a1,n)
    a = a1(1)
  end subroutine allsum_scalar_c16
    

  subroutine allmax_scalar_i(a)
    implicit none
    integer a
    integer a1(1)
    integer n
    ...
    call allmax_i(a1,n)
    a = a1(1)
  end subroutine allmax_scalar_i
    
  subroutine allmax_scalar_r8(a)
    implicit none
    real*8 a
    real*8 a1(1)
    integer n
    ...
    call allmax_r8(a1,n)
    a = a1(1)
  end subroutine allmax_scalar_r8
  ...
  subroutine allsum_r8(a, n)
    ...
    call mpi_allreduce(a, b, n, mpi_double_precision, &
         mpi_sum, mpi_comm_world, ir);if(ir/=0) stop ' alsm '
  ...
  subroutine allsum_c16(a, n)
    ...
    allocate(b(n),stat=ir);  if(ir/=0) stop ' bals '
    call sync_mpi
    call mpi_allreduce(a, b, n, mpi_double_complex, &
         mpi_sum, mpi_comm_world, ir);if(ir/=0) stop ' alsm '
    call sync_mpi
    a=b
    deallocate(b)
  ...
  subroutine allmax_i(a, n)
    ...
    allocate(b(n),stat=ir);  if(ir/=0) stop ' bals '
    call sync_mpi
    call mpi_allreduce(a, b, n, mpi_integer, mpi_max, mpi_comm_world, ir);if(ir/=0) stop ' alsm '
    call sync_mpi
    a=b
    deallocate(b)
  end subroutine allmax_i

  subroutine allmax_r8(a, n)
    implicit none
    integer n, ir,i
    real*8  a(n)
    real*8, allocatable :: b(:)
    ...
    call sync_mpi
    call mpi_allreduce(a, b, n, mpi_double_precision, &
         mpi_max, mpi_comm_world, ir);if(ir/=0) stop ' alsm '
    call sync_mpi
    a=b
    deallocate(b)
  end subroutine allmax_r8
  ...
  subroutine scatter_r8(p, g, n, root)
    ...
    if(nodes>1) then
       call sync_mpi
       call mpi_scatter(p, n, mpi_double_precision, g, n, &
  ...
  ! in alltoall, n2 should equal nodes
  subroutine alltoall_r8(g, p, n1, n2, n3 )
    implicit none
    logical, parameter :: alltoall_debug=.true.

    integer n1,n2,n3,nds, ir, i3
    real*8 g(n1*n2,n3), p(n1*n2,n3)  !
    real*8 sum_p, sum_g, diff

    nds =max(1,nodes)
    call check(n2,nds,' n2nds ')
    
    call sync_mpi
    if(nds>1) then
       do i3=1,n3
          call mpi_alltoall(g(1,i3), n1, mpi_double_precision, &
                            p(1,i3), n1, mpi_double_precision, &
                            mpi_comm_world, ir)
          call check0(ir,' alltoall ')
       enddo
    else
       p=g
    end if
    ...
    !
    !erase -- debug
    if(alltoall_debug)then
       sum_p = sum(p)
       sum_g = sum(g)
       call allsum_scalar_r8(sum_p)
       call allsum_scalar_r8(sum_g)
       diff = abs(sum_p-sum_g)/max(1d-20,abs(sum_p)+abs(sum_g))
       if(diff>1d-8) then
          write(6,*)' problems in allsum, sum_p, sum_g ',sum_p,sum_g
          stop
       end if
    end if

  end subroutine alltoall_r8

  subroutine alltoall_c16(g, p, n1, n2, n3 )
    implicit none
    logical, parameter :: alltoall_debug=.true.

    integer n1,n2,n3,nds, ir, i3
    complex*16 g(n1*n2,n3), p(n1*n2,n3)  !
    complex*16 sum_p, sum_g
    real*8     diff
    ...
    if(nds>1) then
       do i3=1,n3
          call mpi_alltoall(g(1,i3), n1, mpi_double_complex, &
                            p(1,i3), n1, mpi_double_complex, &
                            mpi_comm_world, ir)
          call check0(ir,' alltoall ')
       enddo
    else
    ...
    if(alltoall_debug)then
       sum_p = sum(p)
       sum_g = sum(g)
       call allsum_scalar_c16(sum_p)
       call allsum_scalar_c16(sum_g)
       diff = abs(sum_p-sum_g)/max(1d-20,abs(sum_p)+abs(sum_g))
       if(diff>1d-8) then
          write(6,*)' problems in allsum, sum_p, sum_g ',sum_p,sum_g
          stop
