...
contains
  subroutine read_initiate_seeds
    ...
    if(rank==0) then
       write(6,*) 
       write(6,*)' Now give me in fort.4 from 1 to ',ndiffseeds,' lines with 5 integers each: '
       write(6,*)' First 4 (integer*8) seeds;  last: 1 or not, depending on whether to assign '
...
99  continue
    ...
    line_seed = 1
    !write(6,*)' erase: pre,rank,seed_array ',rank,rank_dependent(1),seed_array(1,line_seed)
    ! initia line, unless otherwise denoted, is 1
    call ran_ps_putseed(seed_array(:,line_seed))

    !write(6,*)' erase: rank,seed_array ',rank,seed_array(1,line_seed)
