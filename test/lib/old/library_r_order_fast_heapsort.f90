...
!*****************************************************         
SUBROUTINE order_r_index(a,i_old_ofnew,n)
  implicit none
  integer n, st
  ...
  allocate(ip(n),ra(n),stat=st) if(st/=0)stop' ip order '

  do i=1,n
     ip(i) = i  ! later, each time we'll reorder we'll also change this array
  enddo
