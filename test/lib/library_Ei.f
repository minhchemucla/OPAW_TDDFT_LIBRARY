! netlib's fn functions, http://www.netlib.org/fn/index.html.  Specifically needing dei.f and de1.f

      double precision function dei (x)
! july 1977 edition.  w. fullerton, c3, los alamos scientific lab.
      double precision x, de1, xin
      real*8, parameter:: bot = -80d0
      external de1
!
      xin=x
      if(xin<bot)xin=bot
...
c
 50   if (x.gt.1.0d0) go to 60
      if (x.eq.0.d0) stop ' x=0 ' !call seteru (14hde1     x is 0, 14, 2, 2)
...
c
 70   if (x.gt.xmax) go to 80
      ...
      return
c
 80   stop ' x so big e1 underflows ' !call seteru (30hde1     x so big e1 underflows, 30, 1, 0)
      de1 = 0.d0
      return
c
      end
