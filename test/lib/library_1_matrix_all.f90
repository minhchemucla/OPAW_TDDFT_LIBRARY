...
!
!        Note that A_evl is NOT the diagonal eigevnlue matrix, D,
!        defined as 
...
!   write(6,*) ' diff from in mat_diag_c16 from hermit. ', diff
  ...
  subroutine ch_diag(A, A_vc, A_evl, m)
    ...
    real*8             :: diff
    real*8, allocatable :: ar(:,:), ai(:,:), w(:), zr(:,:), zi(:,:), fv1(:), fv2(:),fm1(:,:), Unity(:,:)

    complex*16, parameter :: ci = (0.d0,1.d0)
    complex*16 A(m,m), A_vc(m,m), A_evl(m)

    allocate(ar(m,m), ai(m,m), w(m), zr(m,m), zi(m,m), fv1(m),fv2(m),fm1(2,m), Unity(m,m), stat=ierr); 
